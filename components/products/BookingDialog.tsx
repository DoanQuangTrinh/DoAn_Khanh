...
const parsePrice = (priceStr: string): number => parseInt(priceStr?.replace(/[^0-9]/g, ""), 10) || 0;
const formatPrice = (priceNum: number): string => priceNum.toLocaleString("vi-VN") + "đ";

export const BookingDialog = ({ workshopName, data, visible, onHide }: BookingDialogProps) => {
                ...
                <div className="w-full lg:w-1/2 flex flex-col">
                    <div className="mt-6">
                        ...
                        <div className="flex items-center justify-start gap-4">
                            <div className="flex items-center border border-brand-dark rounded-lg">
                                <button onClick={() => setQuantity((q) => (q > 1 ? q - 1 : 1))} className="p-3 text-brand-dark hover:bg-brand-container rounded-l-lg"><MinusIcon /></button>
                                <span className="w-12 text-center text-lg font-bold">{quantity}</span>
                                <button onClick={() => setQuantity((q) => q + 1)} className="p-3 text-brand-dark hover:bg-brand-container rounded-r-lg"><PlusIcon /></button>
                            </div>
                        </div>
                    </div>
                    ...
                    {data?.priceNotes?.length > 0 && (
                        <div className="bg-brand-container p-4 rounded-lg space-y-2 mt-6">
                            {data.priceNotes.map((note, idx) => (
                                <p key={idx} className="text-sm text-brand-text">✨ {note}</p>
                            ))}
                        </div>
                    )}
                    ...
                    <div className="mt-auto pt-6">
                        <div className="flex justify-between items-center border-t border-brand-container pt-4">
                            ...
                            <div className="text-3xl font-bold text-brand-dark">{formatPrice(totalPrice)}</div>
                        </div>
                    </div>
                </div>
            </div>
